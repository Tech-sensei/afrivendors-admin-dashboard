...
import type { Booking, BookingStatus, PaymentStatus } from "./data"
...
export function DrawerFrame({
  ...
  return (
    <>
      ...
      <motion.aside
        className="fixed bottom-0 right-0 top-0 z-1000 hidden w-[600px] max-w-[95vw] flex-col bg-white shadow-2xl md:flex mb-0!"
        initial={{ x: "100%" }}
        animate={{ x: 0 }}
...
export function statusBadge(status: BookingStatus) {
  const styles: Record<BookingStatus, string> = {
    Upcoming: "bg-[#E7EEFF] text-[#2563EB]",
    Pending: "bg-chart-5/15 text-chart-5",
    Accepted: "bg-emerald-500/15 text-emerald-700",
    "In Progress": "bg-chart-5/15 text-chart-5",
    Completed: "bg-chart-2/15 text-chart-2",
    Cancelled: "bg-destructive/15 text-destructive",
    Rejected: "bg-destructive/15 text-destructive",
  }
  const iconMap: Record<BookingStatus, React.ComponentType<{ className?: string }>> = {
    Upcoming: Clock,
    Pending: Clock,
    Accepted: Check,
...
export function paymentBadge(status: PaymentStatus) {
  ...
  return <span className={`inline-flex rounded-full px-3 py-1 font-unageo text-xs font-semibold ${styles[status]}`}>{status}</span>
}
...
export function bookingSearchMatch(booking: Booking, query: string) {
  const q = query.toLowerCase()
  return booking.vendorName.toLowerCase().includes(q)
}
