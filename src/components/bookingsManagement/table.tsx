...
export function BookingsTable({
  ...
  if (isLoading) {
    return <AdminListTableLoading resourceLabel="appointments" />
  }

  if (!bookings.length) {
    return (
      <section className="rounded-xl border border-border bg-white p-10 text-center shadow-sm">
        <Calendar className="mx-auto h-10 w-10 text-accent-60" />
        <h3 className="mt-4 font-unbounded text-xl font-semibold text-secondary-000">No appointments found</h3>
        <p className="mt-1 font-unageo text-sm text-accent-70">Try adjusting status or search.</p>
      </section>
    )
  }
        ...
        <table className="w-full min-w-[1000px]">
          <thead>
            <tr className="border-b border-border bg-secondary-800/50">
              {["Customer", "Vendor", "Service", "Date & Time", "Amount", "Status", "Payment", "Actions"].map((head) => (
                <th
                  key={head}
            ...
            {bookings.map((booking) => (
              ...
              >
                <td className="px-4 py-3">
                  <p className="font-unageo text-sm font-semibold text-secondary-000">{booking.customerName}</p>
                  <p className="mt-0.5 font-unageo text-xs text-accent-70">{booking.customerEmail}</p>
                </td>
                <td className="px-4 py-3 font-unageo text-sm text-secondary-000">{booking.vendorName}</td>
                <td className="px-4 py-3 font-unageo text-sm text-secondary-000">{booking.service}</td>
                <td className="px-4 py-3">
                  <p className="font-unageo text-sm text-secondary-000">{formatShortDate(booking.date)}</p>
                  <p className="mt-0.5 font-unageo text-xs text-accent-70">{booking.time}</p>
                </td>
                ...
                <td className="px-4 py-3">{paymentBadge(booking.paymentStatus)}</td>
      ...
      {filteredCount > 0 ? (
        <div className="flex flex-col items-start justify-between gap-3 border-t border-border p-4 sm:flex-row sm:items-center">
          <p className="font-unageo text-sm text-accent-70">
            Showing {startIndex + 1} to {endIndex} of {filteredCount} appointments
          </p>
          {totalPages > 1 ? (
